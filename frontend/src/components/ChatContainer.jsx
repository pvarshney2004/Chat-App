  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        <img src={selectedUser.profilePic || assets.avatar_icon} className="w-8 rounded-full" alt="Profile" />
        <p className="flex-1 text-md text-white flex items-center gap-2">
          {selectedUser.fullName || 'User Name'}
          {onlineUsers.includes(selectedUser._id) && <span className="rounded-full w-2 h-2 bg-green-500" />}
        </p>
        <img
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 backdrop-blur-lg px-3 rounded-full'>
